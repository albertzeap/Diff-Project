...
void loadlist() {
  ...
  while( i != sizeof(strings1)) {
    slist_pushback(lines1,strings1[i]);
    ++i;
  }
  
  while( j != sizeof(strings2)) {
    slist_pushback(lines2,strings2[j]);
    ++j;
  }
  ...
  while (p != NULL) {
    ...
    if (foundmatch) {
      while ((foundmatch = para_equal(p, q)) == 0) {
        para_print(q, printright_lc);
        q = para_next(q);
        qlast = q;
      }
      para_print(q, printboth_lc);
      p = para_next(p);
      q = para_next(q);
    } else {
      para_print(p, printleft_lc);
      p = para_next(p);
    }
  }
  while (q != NULL) {
...
void suppress() {
  // if ((slist_size(lines1) == 0) || (slist_size(lines2) == 0)) {
  //   fprintf(stderr, "Comparing with empty file\n");
  //   exit(1);
  // }
  // snode* lp1 = lines1->head;
  // snode* lp2 = lines2->head;
  // int i = 0;

  // while( (lp1->data != NULL) && (lp2->data != NULL) ) {
  //   if(strcmp(lp1->data, lp2->data) == 0) {}
...
void init_options_files(int argc, const char* argv[]) {
  ...
  if(suppresscommon){ suppress(); exit(0);}
